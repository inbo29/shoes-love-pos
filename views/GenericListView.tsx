...
const GenericListView: React.FC<GenericListViewProps> = ({ title, buttonLabel, onActionClick, data }) => {
  return (
    <div className="flex-1 flex flex-col p-6 gap-6 overflow-hidden h-full">
      {/* Action Bar */}
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 shrink-0">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white whitespace-nowrap">{title}</h2>
          {buttonLabel && (
            <button 
              onClick={onActionClick}
              className="bg-secondary hover:bg-yellow-400 text-gray-900 font-bold px-5 py-2.5 rounded-xl shadow-md flex items-center gap-2 transform active:scale-95 transition-all text-sm whitespace-nowrap"
            >
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button className="bg-white dark:bg-surface-dark border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 px-4 py-2.5 rounded-xl shadow-sm flex items-center gap-2 transition-colors">
